...
def collect_one_rollout(args):
    ...
    # ── Assign policies per agent ─────────────────────────────────────────────
    # Agent 0: always pure A* → guarantees at least one agent reaches goal
    # Agent 1+: sampled from exploration mix
    ...
    agent_policies = {}
    agent_policies[0] = 'astar'   # guaranteed solver
    for ag in agents[1:]:
        agent_policies[ag] = rng.choice(
            list(EXPLORE_MIX.keys()),
            p=list(EXPLORE_MIX.values()),
        )

    print(f"> Rollout {rollout_idx:04d} | policies: {agent_policies}")

    # Per-agent A* state
    action_queues: Dict[int, List] = defaultdict(list)
    waypoints:     Dict[int, List] = defaultdict(list)
    stuck_counter: Dict[int, int]  = defaultdict(int)
    prev_pos:      Dict[int, Tuple] = {
        ag: tuple(env.unwrapped.agents[ag].state.pos) for ag in agents
    }
    STUCK_THRESH = 5
    ...
    for t in range(max_steps):
        ...
        for ag in agents:
            cur = tuple(env.unwrapped.agents[ag].state.pos)
            if cur == prev_pos[ag]:
                stuck_counter[ag] += 1
            else:
                stuck_counter[ag] = 0
            ...
            # Force replan for any agent that's stuck
            if stuck_counter[ag] >= STUCK_THRESH:
                action_queues[ag].clear()
                waypoints[ag].clear()
                stuck_counter[ag] = 0
        ...
        actions = {}
        for ag in agents:
            policy = agent_policies[ag]

            if policy == 'astar':
                actions[ag] = _astar_action(
                    ag, env.unwrapped, tuple(goal_pos), action_queues)

            elif policy == 'epsilon_astar':
                if env.unwrapped.np_random.random() < 0.3:
                    actions[ag] = env.action_space[ag].sample()
                else:
                    actions[ag] = _astar_action(
                        ag, env.unwrapped, tuple(goal_pos), action_queues)

            elif policy == 'waypoint':
                actions[ag] = _waypoint_action(
                    ag, env.unwrapped, tuple(goal_pos), waypoints)
    ...
    print(f"> Rollout {rollout_idx:04d} | "
          f"len={episode_len:3d} | success={success} | "
          f"policies={agent_policies}")
